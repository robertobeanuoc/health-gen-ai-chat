...
| [`mcp_visualization_health_gen_ai_chat/`](mcp_visualization_health_gen_ai_chat/) | `DashboardEngine` | Validates chart-type/data-volume fit and builds dashboard configs (metrics/charts/tables) from raw data |
| [`chat_agent/`](chat_agent/) | — | LLM agent + web UI. Both terminal mode (`main.py`) and web server (`server.py`) default to Claude Sonnet 4.6, configurable via `CLAUDE_MODEL` in the web server. `streamlit_dashboard.py` is a separate Streamlit process the web UI embeds to render dashboards |
